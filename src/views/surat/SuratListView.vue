...
const columns = [
  { field: 'requestId', header: 'Requesst ID' },
...
const filteredRows = computed(() => {
  ...
  // 2. Search by Ticket ID, Citizen ID, atau jenis layanan
...
// --- Pagination, benar-benar motong hasil filteredRows ---
...
// Reset ke halaman 1 tiap kali filter/search/sort berubah
function resetPage() {
  currentPage.value = 1
}
</script>
...
<template>
  <div>
    <div class="mb-4">
      <h1 class="page-title">Pengelolaan Layanan Surat</h1>
    </div>
    ...
    <div class="card p-4">
      ...
      <!-- Empty state -->
      <p v-if="totalData === 0" class="text-center text-neutral-500 py-6">
      ...
      <div v-else class="flex items-center justify-between mt-4">
        <p class="text-sm text-neutral-500">
          Menampilkan {{ showingFrom }} - {{ showingTo }} dari {{ totalData }} data
        </p>

        <div class="flex items-center gap-1">
          <AppButton icon="pi pi-chevron-left" variant="ghost" size="small"
          ...
          <AppButton icon="pi pi-chevron-right" variant="ghost" size="small"
