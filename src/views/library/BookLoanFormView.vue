...
const toast = useToast(); const cachedBooks = getListCache('books'); const cachedCitizens = getListCache('citizens'); const books = ref(cachedBooks ?? []); const citizens = ref(cachedCitizens ?? []); const loading = ref(!cachedBooks && !cachedCitizens); const saving = ref(false); const loanReceipt = ref(null)
